...
//
// Slightly (minimally) modified by Daniel Owen van Dommelen, for less warping.
var CrtLotteKageMod = []byte(`
//go:build ignore
...
type CrtLotteMod struct {
	BaseShader
}

func (s *CrtLotteMod) Apply(screen *ebiten.Image, buffer *ebiten.Image) error {
	s.Lock()
	defer s.Unlock()
	...
	sop.GeoM.Scale(float64(screen.Bounds().Dx())/float64(buffer.Bounds().Dx()), float64(screen.Bounds().Dy())/float64(buffer.Bounds().Dy()))
	sop.Uniforms = s.Uniforms
	sop.Images[0] = buffer

	screen.Clear()
	screen.DrawRectShader(screen.Bounds().Dx(), screen.Bounds().Dy(), s.Shader, sop)

	return nil
}

func NewCrtLotteMod() (*CrtLotteMod, error) {
	shader, err := ebiten.NewShader(CrtLotteKage)
	if err != nil {
		return nil, err
	}
	...
	s := &CrtLotteMod{
		BaseShader: BaseShader{
			...
			Uniforms: map[string]interface{}{
				...
				"WarpX":         float32(0.0),
				"WarpY":         float32(0.0),
				"MaskDark":      float32(0.5),
				"MaskLight":     float32(1.5),
				"ShadowMask":    float32(3.0),
				"BrightBoost":   float32(1.0),
				"HardBloomPix":  float32(-1.5),
				"HardBloomScan": float32(-2.0),
				"BloomAmount":   float32(1.0),
				"Shape":         float32(1.0),
			},
		},
	}
